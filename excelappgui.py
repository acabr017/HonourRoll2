...
def get_files_func():
    global file_name
    file_name = list(filedialog.askopenfilenames(initialdir=get_files_path,title="Select File(s)"))
    ...
    for file in file_name:
        if file.endswith(".xls") or file.endswith(".xlsx"):
            ...
            messagebox.showerror("Warning!","Only .xls and .xlsx files accepted!",parent=window) # To do: figure out how to get this warning on the window itself
...
def get_output_files():
    return [fn for fn in os.listdir(output_location) if fn.lower().endswith(output_type) and os.path.isfile(os.path.join(output_location, fn))]
...
def populate_output_table():
    ...
    for file in output_files:
        insert_to_output_table(clean_string_for_table_insert(file),f"{output_location}/{file}")
...
# Instructions Frame
instructions_frame = tk.Frame(window,borderwidth=5,width=570,height=150,relief='groove')
# instructions_frame.pack_propagate(False)

# Setup Buttons Frame
setup_buttons_frame = tk.Frame(window,borderwidth=5,width=int(window_width*1.2),height=100)
setup_buttons_frame.pack_propagate(False)

# Output Files Label Frame
output_files_label_frame = tk.Frame(window,width=int(window_width*1.2),height=50,relief='groove')
setup_buttons_frame.pack_propagate(False)
...
# Table Label Frame
table_label_frame = tk.Frame(placeholder_frame,width=int(window_width*1.2),height=40, relief='groove')
...
# Get File Button
get_files_button = tk.Button(setup_buttons_frame,text="Upload Files",command=get_files_func,font=('Times New Roman',14))

# File Delete Button
file_delete_button = tk.Button(del_and_start_buttons_frame,text="                   Delete                   ",state="disabled",command=lambda: delete_items(""),font=('Times New Roman',14))

# Set Directory Button
set_directory_button = tk.Button(setup_buttons_frame,text="Set Directory For Output Files",command=set_directory_func,font=('Times New Roman',14))

# Start Button
start_button = tk.Button(del_and_start_buttons_frame,text="                    Start                    ",state='disabled',command=start_button_func,font=('Times New Roman',14))

# Radio Buttons for Output Type:
output_type_string = tk.StringVar()


txt_radio_button = ttk.Radiobutton(radio_frame,text="Text Output",value="txt", variable=output_type_string,command=radio_button_func)
csv_radio_button = ttk.Radiobutton(radio_frame,text="CSV Output",value='csv',variable=output_type_string,command=radio_button_func)
...
# input_scrolllH = ttk.Scrollbar(input_table_frame,orient='horizontal',command=input_table.xview)
# input_table.configure(xscrollcommand=input_scrolllH)
...
output_labels_frame_label = tk.Label(output_files_label_frame,text="Files will be saved to:")
directory_label = tk.Label(output_files_label_frame,textvariable=directory_label_string)
...
instructions_label1 = tk.Label(instructions_frame,justify='left',text=instructions_text1,font=("Times New Roman",14))
instructions_label2 = tk.Label(instructions_frame,justify='center',text=instructions_text2,font=('Times New Roman', 12))


# Table Label

table_label = tk.Label(table_label_frame,justify='left',text='Selected Files',font=('Times New Roman',11))
